...
namespace OrbitSets {
	...
	bool operator==(variant<Ts...> a, variant<Ts...> b) {
		if (a.current_index != b.current_index) {
			...
			return __type_helper<Ts...>::eq(a.current_index, &(a.data), &(b.data));
		}
	}
	template<typename... Ts>
	bool operator!=(variant<Ts...> a, variant<Ts...> b) {
	...
	bool operator<(variant<Ts...> a, variant<Ts...> b) {
		...
		if (a.current_index > b.current_index)	
			return false;
		return __type_helper<Ts...>::lt(a.current_index, &(a.data), &(b.data));
	}
	template<typename... Ts>
	bool operator>(variant<Ts...> a, variant<Ts...> b) {
		return b < a;
	...
	std::ostream& operator<<(std::ostream& o, variant<Ts...> a) {
		return __type_helper<Ts...>::output(a.index(), o, &(a.data));
	}
	...
	public:
		~variant() {
			if (current_index != __temparg_count<TF, Ts...>::v) {
				__type_helper<TF, Ts...>::d(current_index, &data);
			}
		}

		variant() {
			current_index = __temparg_count<TF, Ts...>::v;
		}
	
		variant(const variant<TF, Ts...>& orig) {
			current_index = __temparg_count<TF, Ts...>::v;
			if (orig.current_index != current_index) {
				__type_helper<TF, Ts...>::c(orig.current_index, &data, &(orig.data));
		...
		variant(variant<TF, Ts...>&& orig) {
			current_index = __temparg_count<TF, Ts...>::v;
			if (orig.current_index != current_index) {
				__type_helper<TF, Ts...>::m(orig.current_index, &data, &(orig.data));
				current_index = orig.current_index;
				orig.current_index = __temparg_count<TF, Ts...>::v;
			}
		}
		...
		variant(variant<Ts2...> orig) {
			current_index = __temparg_count<TF, Ts...>::v;
			__type_helper<Ts2...>::c(orig.index(), orig.data)
			*this = __extract_helper<variant<TF, Ts...>, variant<Ts2...>, Ts2...>::extract(orig.index(), orig);
		}
		...
		template<unsigned n>
		typename __temparg_extract<n, TF, Ts...>::type& get() {
			return get<typename __temparg_extract<n, TF, Ts...>::type>();
		}
	
		template<typename T>
		T& get() {
			if (current_index != __temparg_index<T, TF, Ts...>::v)
				throw std::bad_cast();
			return *reinterpret_cast<T*>(&data);
		}
	
		template<unsigned n>
		void set(typename __temparg_extract<n, TF, Ts...>::type&& value) {
			set<typename __temparg_extract<n, TF, Ts...>::type>(std::move(value));
		}
