...
private:
	typedef typename __typename_orbitize<Ts...>::type orbitTypestore;
	typedef typename __typename_store_variant<orbitTypestore>::type orbitVariant;
...
public:
	...
	orbit() {}
	orbit(variant<Ts...> el) {
		Vorbit = __variant_orbit_helper<orbitVariant, variant<Ts...>, Ts...>::createFromElement(el.index(), el);
	}
	template<typename T>
	orbit(orbit<T> orig) {
		Vorbit = orig;
